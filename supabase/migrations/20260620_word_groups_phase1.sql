-- Phase 1 Word Groups migration
-- Generated from reviewed Step 2 design.

create extension if not exists "pgcrypto";

-- Keep helper aligned with existing schema behavior.
...
create table if not exists public.word_groups (
  ...
  constraint word_groups_grade_check check (
    upper(grade) in ('P1', 'P2', 'P3', 'P4', 'P5', 'P6', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6')
  ),
...
drop trigger if exists word_groups_set_updated_at on public.word_groups;
...
drop trigger if exists user_group_preferences_set_updated_at on public.user_group_preferences;
...
drop policy if exists "Authenticated users can read word_groups" on public.word_groups;
create policy "Authenticated users can read word_groups"
on public.word_groups for select
using (auth.uid() is not null);

drop policy if exists "Owner/admin can insert word_groups" on public.word_groups;
...
drop policy if exists "Owner/admin can update word_groups" on public.word_groups;
...
drop policy if exists "Owner/admin can delete word_groups" on public.word_groups;
create policy "Owner/admin can delete word_groups"
on public.word_groups for delete
using (auth.uid() is not null and public.is_owner_or_admin());

drop policy if exists "Authenticated users can read wordbase_group_map" on public.wordbase_group_map;
create policy "Authenticated users can read wordbase_group_map"
on public.wordbase_group_map for select
using (auth.uid() is not null);

drop policy if exists "Owner/admin can insert wordbase_group_map" on public.wordbase_group_map;
create policy "Owner/admin can insert wordbase_group_map"
on public.wordbase_group_map for insert
with check (auth.uid() is not null and public.is_owner_or_admin());

drop policy if exists "Owner/admin can update wordbase_group_map" on public.wordbase_group_map;
create policy "Owner/admin can update wordbase_group_map"
on public.wordbase_group_map for update
using (auth.uid() is not null and public.is_owner_or_admin())
with check (auth.uid() is not null and public.is_owner_or_admin());

drop policy if exists "Owner/admin can delete wordbase_group_map" on public.wordbase_group_map;
create policy "Owner/admin can delete wordbase_group_map"
on public.wordbase_group_map for delete
using (auth.uid() is not null and public.is_owner_or_admin());

drop policy if exists "Users can read own picks, owner/admin can read all" on public.user_group_picks;
...
drop policy if exists "Users can insert own picks, owner/admin can insert all" on public.user_group_picks;
...
drop policy if exists "Users can delete own picks, owner/admin can delete all" on public.user_group_picks;
...
drop policy if exists "Users can read own preferences, owner/admin can read all" on public.user_group_preferences;
...
drop policy if exists "Users can insert own preferences, owner/admin can insert all" on public.user_group_preferences;
...
drop policy if exists "Users can update own preferences, owner/admin can update all" on public.user_group_preferences;
...
drop policy if exists "Users can delete own preferences, owner/admin can delete all" on public.user_group_preferences;
